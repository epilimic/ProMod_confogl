...
public Plugin:myinfo = 
{
	...
	version = "2.8_pm", //recolored for promod is the only difference
...
public Action:ToggleSpecHudCmd(client, args) 
{
	...
	CPrintToChat(client, "Spectator HUD is now %s.", (bSpecHudActive[client] ? "{lightgreen}on{default}" : "{lightgreen}off{default}"));
}

public Action:ToggleTankHudCmd(client, args) 
{
	bTankHudActive[client] = !bTankHudActive[client];
	CPrintToChat(client, "Tank HUD is now %s.", (bTankHudActive[client] ? "{lightgreen}on{default}" : "{lightgreen}off{default}"));
}
...
public Action:HudDrawTimer(Handle:hTimer) 
{
	...
	if (bSpecsOnServer) // Only bother if someone's watching us
	{
		...
		for (new i = 1; i <= MaxClients; i++) 
		{
			...
			if (!bSpecHudHintShown[i])
			{
				...
				CPrintToChat(i, "Type {green}!spechud{default} into chat to toggle the {lightgreen}Spectator HUD{default}.");
			}
		}
	...
	for (new i = 1; i <= MaxClients; i++) 
	{
		...
		if (!bTankHudHintShown[i])
		{
			...
			CPrintToChat(i, "Type {green}!tankhud{default} into chat to toggle the {lightgreen}Tank HUD{default}.");
		}
	}
